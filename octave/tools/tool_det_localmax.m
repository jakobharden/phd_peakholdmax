...
##
## Usage: [r_max, r_idx] = tool_det_localmax(p_xx, p_ni, p_cl, p_dl)
##
## p_xx  ... signal amplitude array, [<dbl>]
## p_ni  ... detection window start index, <uint>
## p_cl  ... hold-peak counter limit, <uint>
## p_dl  ... detection window length, optional, default = |x| - p_ni + 1, <uint>
...
##     step 6.2: evaluate stop-condition, condition: c == p_cl; true: r_max = a, r_idx = p_cl - n, break loop; false: next loop
##
#######################################################################################################################
## LICENCE
##
...
#######################################################################################################################
##
function [r_max, r_idx] = tool_det_localmax(p_xx, p_ni, p_cl, p_dl)
  ...
  if (nargin < 3)
    help tool_det_localmax;
    error('Less arguments given!');
  endif
  
  ## step 1: determine global maximum value and sample index (init return values)
  [r_max, k_globmax] = max(p_xx(p_ni : end)); # global maximum value and local sample index
  r_idx = k_globmax + p_ni - 1; # map local to global sample index
  
  ## step 2: determine maximum detection window length
  dl_max = numel(p_xx) - p_ni + 1;
  ...
  a = min(p_xx(p_ni : end)); # temporary maximum amplitude, use global minimum
  c = 0; # hold-peak counter
  
  ## step 6: loop over signal samples within detection window
  for n = p_ni : (p_ni + p_dl - 1)
